def make_list(films: dict, header):
    text = header
    for ticked in films['ticked']:
        text += ticked + ticked_mark + '\n\n'
    for unticked in films['unticked']:
        text += unticked + unticked_mark + '\n\n'
    return text


start_text = '''
Доброго времени суток!
Я Ваш персональный бот-киноман, который поможет Вам добавлять фильмы в список, \
отмечать их в таблице, \
а также показывать доступные Вам и \
предлагать случайный фильм на выбор. \
И это далеко не все
Для начала, нужно войти в систему
Нажмите /login для входа в аккаунт обычного пользователя, либо /admin для входа в аккаунт администратора
'''

login_user_text = '''
Выберите аккаунт для авторизации:
'''

login_password_text = '''
Введите пароль
'''

bad_password_text = '''
Неверный пароль, нажмите /login, чтобы повторить попытку, либо /stop_auth, чтобы выйти
'''

echo_auth_text = '''
Пожалуйста, продолжите авторизацию, либо нажмите /stop_auth, чтобы выйти
'''

stop_auth_text = '''
Авторизация прервана
'''

unauthorized_text = '''
Вы не подтвердили свою личность

Пожалуйста, нажмите /login или /admin для авторизации
'''

logged_in_text = '''
Вы уже в системе
'''

log_out_text = '''
Вы успешно вышли из системы
'''

error_text = '''
Ошибка авторизации
'''

bad_auth_text = '''
Данный пользователь уже вошел в систему
Пожалуйста, выберите друго пользователя
'''

echo_text = '''
Нажмите /help для получения списка доступных команд
'''

auth_text = '''
Добро пожаловать, {}!
Нажми /help для просмотра возможностей бота
'''

help_text = '''
Список доступных команд:
/random - выбрать случайный фильм из списка
/tick - отметить фильм
/untick - убрать отметку фильма
/add - добавить фильмы в свой список
/remove - убрать фильмы из своего списка
/list - открыть список своих фильмов
/list_user - открыть список фильмов другого пользователя

Дополнительно:
/help - узнать список возможностей
/log_out - выйти из системы
<a href = "{}">table_link</a> - открыть таблицу в браузере 
'''

random_film_process_text = '''
Выбирается случайный фильм...
{}
'''

random_film_text = '''
Ваш фильм: {}
'''

tick_text = '''
Пожалуйста, выберите фильмы, которые вы хотите отметить и нажмите <em>закончить</em>
'''

untick_text = '''
Пожалуйста, выберите фильмы, с которых вы хотите снять отметку и нажмите <em>закончить</em>
'''

completed_text = '''
Операция успешно выполнена!
'''

ticked_mark = '''  — ✅'''

unticked_mark = '''  — ❌'''

removed_mark = '''  — 🚫'''

complete_button = '''закончить  — 🆗'''

need_to_complete_text = '''
Пожалуйста, выберите фильмы и нажмите <em>закончить</em>
'''

add_text = '''
Введите названия фильмов, которые вы хотите добавить 
Название каждого фильма должно быть на отдельной строке
'''

remove_text = '''
Выберите фильмы для удаления, а затем нажмите <em>закончить</em>
'''


self_films_list_text = '''
Список Ваших фильмов:

'''

enter_username_list_text = '''
Выберите пользователя, чей список вы хотите посмотреть 
'''

echo_username_text = '''
Пожалуйста, выберите пользователя 
'''

user_films_list_text = '''
Список фильмов пользователя {}:

'''

auth_admin_text = '''
Введите Ваш идентификатор администратора и пароль

ВНИМАНИЕ: ввод должен быть в следующем формате:
username
password1234
'''

auth_admin_error_text = '''
Ошибка: неправильный логин или пароль 

Проверьте корректность вводимых данных и формат ввода
Введите данные еще раз, либо нажмите /stop_auth, чтобы вернуться на главную страницу
'''

help_admin_text = '''
Список доступных команд:
/all_users - список всех пользователей
/add_user - добавить нового пользователя
/change_user - изменить данные пользователя
/remove_user - удалить пользователя
/disconnect_user - отключить пользователя
/disconnect_all_users - отключить всех пользователей
/format - изменить формат таблицы (временно недоступно)

Дополнительно:
/help_admin - посмостреть список всех команд администратора
/exit - выход на главный экран
'''

exit_admin_text = '''
Вы вышли из панели администрирования
'''

echo_admin_text = '''
Для просмотра списка доступных команд, нажмите /help_admin
'''

users_list_admin_text = '''
Список всех добавленных пользователей:

'''

user_authorized_mark = ''' - <b>подключен</b>'''

user_unauthorized_mark = ''' - <b>не подключен</b>'''

add_user_admin_text = '''
Введите имя пользователя и пароль

ВНИМАНИЕ: ввод должен быть в следующем формате:
username
password1234
'''

done_admin_text = '''
Изменения успешно внесены
'''

user_data_error_admin_text = '''
Ошибка ввода. Возможные причины:
  ·  Данные не соответствуют формату
  ·  Такой пользователь уже существует
  ·  Имя пользователя/пароль не изменились

Проверьте входные данные и попробуйте еще раз, либо нажмите /stop_admin
'''

stop_admin_text = '''
Операция прервана
'''

username_button = '''
Имя пользователя
'''

password_button = '''
Пароль
'''

choose_user_admin_text = '''
Выберите пользователя для изменения
'''

bad_chg_user_admin_text = '''
Данный пользователь не может быть изменен, либо у вас нет доступа

Проверьте данные и повторите попытку, либо нажмите /stop_admin
'''

change_user_admin_text = '''
Выберите поле для изменения
'''

new_field_admin_text = '''
Введите новое значение поля
'''

error_admin_text = '''
Пожалуйста, продолжите ввод, либо нажмите /stop_admin
'''

to_rm_user_admin_text = '''
Выберите пользователя для удаления
'''

bad_rm_user_admin_text = '''
Данный пользователь не может быть удален, либо у вас нет доступа

Проверьте данные и повторите попытку, либо нажмите /stop_admin
'''

confirm_rm_admin_text = '''
Требуется подтверждение удаления пользователя

Введите свой пароль администратора и пароль пользователя

ВНИМАНИЕ: ввод должен быть в следующем формате:
admin_username
admin_password
user_password
'''

bad_password_rm_user_admin_text = '''
Ошибка ввода:
Введенные данные некорректны

Проверьте входные данные и попробуйте еще раз, либо нажмите /stop_admin
'''

selected_to_rm_user_admin_text = '''
Удаление пользователя {}...
'''

disconnect_user_admin_text = '''
Выберите пользователя для отключения
'''

bad_disconnect_user_admin_text = '''
Данный пользователь не может быть отключен

Выберите другого пользователя, либо нажмите /stop_admin
'''

confirm_button = '''Подтвердить'''

cancel_button = '''Отмена'''

confirm_disconnection_admin_text = '''
Подтвердите отключение пользователей
'''

disconnection_alert_admin_text = '''
Вы были отключены от системы администратором
'''

removal_alert_admin_text = '''
Вы были удалены от системы администратором
'''

fatal_error_text = '''
Произошла непредвиденная ошибка

Повторите попыку позже
'''

no_users_authorized_admin_text = '''
На данный момент в системе нет авторизированных пользователей
'''